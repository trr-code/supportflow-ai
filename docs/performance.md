...
Historical GET/chat benches targeted `https://supportflow-ai-ou1b5gvy.on-forge.com` on the shared CareerForge VM (1 vCPU, 961 MiB), Octane/FrankenPHP **1 worker** on `127.0.0.1:8000` behind Nginx TLS. Live staging is now `https://supportflow-ai-lqojsjr5.on-forge.com` with **1 worker** on `127.0.0.1:8001`. GET benches use the HTTPS URL, so they do not depend on that loopback port. Port **8001** is permanent. Scheduler was not paused for the 19 Sep run. GET benches used Pest Stressless only (`K6_NO_COOKIES_RESET=true`, `STRESS_MAX_CONCURRENCY=16`). Chat used one-shot `POST /chat/stream` with separate cookie jars; `throttle:chat` stayed at 10/minute.
...
Forge staging is off-peak only: `https://supportflow-ai-lqojsjr5.on-forge.com`. Start with smoke. Stop if it fails. GET `/up`, `/`, `/knowledge`, and `/knowledge/return-window` only.
